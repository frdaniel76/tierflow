...
- Auth types: `env` (environment variables), `file`, `keychain` (macOS), `profiles` (JSON profiles), `none` (local providers), per-provider overrides
...
- **Proxy server** — zero-dependency HTTP server exposing OpenAI-compatible `/v1/chat/completions`
- **Provider translation** — Anthropic Messages API ↔ OpenAI format (streaming, tool calls, thinking blocks)
- **Auth module** — flexible API key management (env vars, files, keychain, profiles)
- **14-dimension weighted classifier** — rule-based routing across token count, code presence, reasoning markers, technical terms, creative markers, and 9 other dimensions
- **4-tier routing** — SIMPLE, MEDIUM, COMPLEX, REASONING with configurable model mappings
- **Fallback chains** — automatic retry with fallback model on failure
- **Adaptive thinking** — auto-configures thinking budgets per model
